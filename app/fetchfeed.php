...
foreach($posts as $post) { ?>
	<div class="card block" id="post<?php echo $post['post_id'] ?>">
		<div class="card-content">
			...
			<div class="card-content">
				<span class="icon is-medium <?php if (in_array($post['post_id'], $likes)) echo 'is-hidden'; ?>" onclick=toggleLike(this.id) id="like<?php echo $post['post_id'] ?>">
					<svg style="width:24px;height:24px" viewBox="0 0 24 24">
						<path fill="currentColor" d="M12.1 18.55L12 18.65L11.89 18.55C7.14 14.24 4 11.39 4 8.5C4 6.5 5.5 5 7.5 5C9.04 5 10.54 6 11.07 7.36H12.93C13.46 6 14.96 5 16.5 5C18.5 5 20 6.5 20 8.5C20 11.39 16.86 14.24 12.1 18.55M16.5 3C14.76 3 13.09 3.81 12 5.08C10.91 3.81 9.24 3 7.5 3C4.42 3 2 5.41 2 8.5C2 12.27 5.4 15.36 10.55 20.03L12 21.35L13.45 20.03C18.6 15.36 22 12.27 22 8.5C22 5.41 19.58 3 16.5 3Z" />
					</svg>
				</span>
				<span class="icon is-medium <?php if (!in_array($post['post_id'], $likes)) echo 'is-hidden'; ?>" onclick=toggleLike(this.id) id="unlike<?php echo $post['post_id'] ?>">
					<svg style="width:24px;height:24px" viewBox="0 0 24 24">
				...
				<span class="icon is-medium"  onclick=goToComment(this.parentNode) id="comment_icon<?php echo $post['post_id'] ?>">
					<svg style="width:24px;height:24px" viewBox="0 0 24 24">
				...
				<p class="block">
					<a class="clickable has-text-black" href="/profile?user=<?php echo $post['username'] ?>">
						<strong class="mr-3"><?php echo $post['username'] ?></strong></a><?php echo $post['description'] ?></p>
				<p class="block">
					<time datetime="<?php echo $post['creation_date'] ?>"><?php echo $post['creation_date'] ?></time>
				</p>
				<p class="block clickable" id="show_comments<?php echo $post['post_id'] ?>" onclick=showComments(this)>
					<strong>
	<?php
	if ($post['comments'] == 1)
	...
	if ($post['comments'] > 1)
		echo 'Show all ' . $post['comments'] . ' comments';?>
				...
				<div class="media-left">
					<figure class="image is-48x48">
						<img src="/getimage?name=<?php echo $_SESSION['profile_image'] ?>" alt="Profile picture">
					</figure>
				</div>
				<div class="media-content">
					<input class="input" type="text" id="comment<?php echo $post['image_file'] ?>" placeholder="Write a comment">
